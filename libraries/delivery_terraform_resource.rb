...
class Chef
  class Resource
    class DeliveryTerraForm < Chef::Resource
      ...
      def initialize(name, run_context = nil)
        ...
        @provider = Chef::Provider::DeliveryTerraForm
        @repo_path = delivery_workspace_repo

        @action = :test
        %w(init plan apply destroy show test).each do |a|
          @allowed_actions.push(a.to_sym)
        end
      end

      #
      # The directory path to the directory where the *.tf files are located
      #
      def plan_dir(arg = nil)
        set_or_return(
          :plan_dir,
          arg,
          kind_of: String,
